...
var myLink = (() => {
  ...
  let userProfileReq = new Request(
    MyLinkConfig.currenturl + MyLinkConfig.userprofileurl,metaData
  );
    ...
    .then(data => {
      ...
      customProperties.map(item => {
        ...
        if (userProfileProperty == null)
          userProperties[userProfileProperty.Key] = null; // deepscan-disable-line NULL_POINTER
        else {
...
let getLayoutsPageUrl=function(libraryName){
  if (window
...
let getParentTerm = function() {
  ...
  function fetchTaxonomy() {
    ...
    SP.SOD.registerSod("sp.taxonomy.js", getLayoutsPageUrl("sp.taxonomy.js"));
